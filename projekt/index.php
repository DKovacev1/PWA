    ...
    <body>
        ...
        <?php 
            ...
            if($conn){
                ...
                $sevaKategorija = "Ševa";
                $stmt->bind_param("s", $sevaKategorija );        
                $stmt->execute();
                $result1 = $stmt->get_result();



                $stmt2 = $conn->prepare("SELECT id, naslovVijesti, nazivSlike FROM novosti 
                                        WHERE kategorija = ?
                                        AND arhiva = 0
                                        ORDER BY datumObjave DESC
                                        LIMIT 3;");

                $ostatakKategorija = "Ostatak ekipe";
                $stmt2->bind_param("s", $ostatakKategorija );        
                $stmt2->execute();
                        ...
                        <?php 
                            ...
                            while ($row2 = $result2->fetch_assoc()) {
                                ...
                                echo "<div class='col-lg-4 col-md-6'>";
                                    ...
                                    echo "<a href=\"clanak.php?id=". urlencode($row2['id']) . " \" >";

                                        echo "<div class='image-container' >";
                                            echo "<img src='img/" . $row2['nazivSlike'] . "' alt=\"\">";
                                        echo "</div>";

                                        echo "<h4>" .  $row2['naslovVijesti'] . "</h4>";
